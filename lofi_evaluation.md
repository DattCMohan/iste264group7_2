...
Overall, we decided to keep going in a similar direction for our hifi prototype, but make some refinements. Most of the responses to the questions on the google form were generally positive and users were able to complete all the assigned tasks. However, there were still some errors made when doing the tasks and confusion when using the prototype. Some user flows took too long or had too many clicks for users to to complete their goal, so there are definitely some improvements/changes we should make based on this feedback.
